...
function calcStandard(items: Record<string, { score?: number }>): number {
  ...
  return Math.round(
    (vals.reduce((a, b) => a + b, 0) / (vals.length * 3)) * 100
  );
}

function calcBehavioral(
  items: Record<string, { present?: boolean; intensity?: number }>
): number {
  ...
  return Math.round(
    (scores.reduce((a: number, b: number) => a + b, 0) / (scores.length * 3)) *
      100
  );
}

function calcSensory(
  modalities: Record<string, { pattern?: string; severity?: number }>
): number {
...
export async function createAssessment(
  ...
  if (user.role !== "therapist") {
    throw new AppError("FORBIDDEN", "Only therapists can create assessment");
  }
  ...
  const snapshot = {
    ...
    developmental_quotient: scores.developmental_quotient,
  ...
  if (!assessment) {
    ...
    assessment.previous = assessment.latest;

    assessment.latest = snapshot;
...
export async function updateLatestAssessment(
  ...
  if (user.role !== "therapist") {
    throw new AppError("FORBIDDEN", "Only therapists can update assessment");
  }
...
export async function listAssessments(childId: string, user: AuthPayload) {
  await assertAccess(childId, user);
  return AssessmentModel.find({ child_id: childId }).sort({ createdAt: -1 });
}
